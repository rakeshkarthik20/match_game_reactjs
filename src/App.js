...
class App extends Component {
  ...
  tick = () => {
    ...
    if (sec !== 0) {
      ...
      clearInterval(this.timer)
      this.setState({sec: 60, active: false})
    }
  }

  onClickChangeTabItems = e => {
  ...
  onClickCheckImage = e => {
    ...
    const updatedImageId =
      imagesList[Math.floor(Math.random() * imagesList.length)].id

    this.setState({
      score: updateScore,
      imgID: updatedImageId,
    })
  }

  render() {
    const {category, score, imgID, sec, active} = this.state
    const updatedImagesList = imagesList.filter(
      each => each.category === category,
    )

    const updatedMainImage = imagesList.filter(each => each.id === imgID)[0]
      .imageUrl

    return (
      <div className="main-container">
        <ul className="navbar-container header">
          <li>
            <img
          ...
          <li className="scores-container">
            ...
            <div className="timer-container">
              ...
              />
              <p className="time-text">{sec} sec</p>
            </div>
          </li>
        </ul>
        {active && (
          <div className="sub-container">
            <div className="main-image-container">
              ...
              <ul className="tabsList-container">
                {tabsList.map(each => (
                  <li id={each.tabId} className="tabItem " key={each.tabId}>
                    <button
                      type="button"
                      className="tabItemButton"
                      onClick={this.onClickChangeTabItems}
                      id={each.tabId}
                    >
                      {each.displayText}
                    </button>
              ...
              <ul className="thumbImageList-container">
                {updatedImagesList.map(each => (
                  <li id={each.id} className="thumbImageItem" key={each.id}>
                    <button
                      type="button"
                      className="image-button"
                      onClick={this.onClickCheckImage}
                      id={each.id}
                    >
                      <img
        ...
        {!active && (
          <div className="Scorecard-container scorecard">
            <img
